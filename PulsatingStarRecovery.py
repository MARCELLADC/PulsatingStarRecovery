...
class PulsatingStarRecovery(maf.BaseMetric):
    ...
    is able to recover the period and the shape of a light curve (from a template given in ascii file)
    ...
    def __init__(self,filename,dmod,sigmaFORnoise,do_remove_saturated,numberOfHarmonics,factorForDimensionGap,mjdCol='observationStartMJD',fiveSigmaDepth='fiveSigmaDepth',
                 filters= 'filter' ,night='night',visitExposureTime='visitExposureTime',skyBrightness='skyBrightness', numExposures='numExposures', seeing='seeingFwhmEff',airmass='airmass',**kwargs):
        ...
        super().__init__(col=cols, maps=maps, units='#', **kwargs, metricName="PulsatingStarRecovery", metricDtype="object")
    ...
    def run(self, dataSlice, slicePoint=None):
        ...
        #the functionn 'ReadTeoSim' puts the pulsating star at distance=dmod+Ax/Av*3.1*slicePoint('ebv').Gives a dictionary
        ebv1=slicePoint['ebv']
        ...
        #here we build mv that will be used in our metrics to build the siulated light curve:
        ...
        best_per_temp,diffper,diffper_abs,diffcicli=self.LcPeriod(mv,LcTeoLSST,LcTeoLSST_noised,index_notsaturated)
        ...
        deltamag_u=lcModel['meanu']-finalResult['mean_u']
        deltamag_g=lcModel['meang']-finalResult['mean_g']
        deltamag_r=lcModel['meanr']-finalResult['mean_r']
        deltamag_i=lcModel['meani']-finalResult['mean_i']
        deltamag_z=lcModel['meanz']-finalResult['mean_z']
        deltamag_y=lcModel['meany']-finalResult['mean_y']
        #the same can be done for the amplitudes (without the effect of blending for the momment.)
        deltaamp_u=lcModel['amplu']-finalResult['ampl_u']
        deltaamp_g=lcModel['amplg']-finalResult['ampl_g']
        deltaamp_r=lcModel['amplr']-finalResult['ampl_r']
        deltaamp_i=lcModel['ampli']-finalResult['ampl_i']
        deltaamp_z=lcModel['amplz']-finalResult['ampl_z']
        deltaamp_y=lcModel['amply']-finalResult['ampl_y']
        ...
        output_metric={'n_u':uni_meas['n_u'],'n_g':uni_meas['n_g'],'n_r':uni_meas['n_r'],'n_i':uni_meas['n_i'],'n_z':uni_meas['n_z'],'n_y':uni_meas['n_y'],
                ...
                'deltamag_u':deltamag_u, 'deltamag_g':deltamag_g,'deltamag_r':deltamag_r,'deltamag_i':deltamag_i,'deltamag_z':deltamag_z,'deltamag_z':deltamag_z,
                'deltaamp_u':deltaamp_u, 'deltaamp_g':deltaamp_g,'deltaamp_r':deltaamp_r,'deltaamp_i':deltaamp_i,'deltaamp_z':deltaamp_z,'deltaamp_z':deltaamp_z,
...
#    return time_model,phase_model,u_model,g_model,r_model,i_model,z_model,y_model
    ...
    def LcPeriod(self,mv,LcTeoLSST,LcTeoLSST_noised,index_notsaturated):
        """
        ...
        #Step's choice
        periods = np.linspace(minper_plot, maxper_plot,1000)
...
#        self.plotting(data,fitting,period,label,zeroTimeRef)
